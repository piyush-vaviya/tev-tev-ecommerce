...
async function main () {
  logger.info('CRON Deleting sellables start...')
  ...
  try {
    async.whilst(async () => hasNexPage, async () => {
      ...
      const products = await Product.find({
        es_status: APP_CONSTANTS.ELASTICSEARCH_SYNC_STATUS.DELETE,
      })
      ...
      if (products.length > 0) {
        ...
        // mark products as being deleted
        await Product.updateMany({ _id: { $in: productIds } }, { es_status: APP_CONSTANTS.ELASTICSEARCH_SYNC_STATUS.DELETING })
      ...
      logger.info('CRON Deleting sellables end...')
      process.exit(0)
    })
  } catch (err) {
    logger.error(err)
